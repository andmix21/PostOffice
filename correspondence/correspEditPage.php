...
$corresp = get_corresp_Info_By_ID($_GET['correspEditById']);
...
<head>
    ...
    <title>Редактирование данных корреспонденции</title>
</head>
<body>
    <section class = "beginning", id='home'>
        <div class = "title_text">РЕДАКТИРОВАНИЕ ДАННЫХ КОРРЕСПОНДЕНЦИИ</div>
    </section>
    <div class="nav">
        <ul>
    ...
    <section class = "formSection">
        <form action = "correspEditByIdController.php" method = "POST" role = 'form'>
            <input id="id" type="hidden" name="id" value="<?php echo $_GET['correspEditById'];?>"/>
            <div class = form>
                        ...
                        <select id = "corresp_type_id" name = "corresp_type_id">
                            <?php
                                ...
                                for ($i = 0; $i < count($corresp_type_info); $i++)
                                {
                                    ...
                                    {$a = 'selected';}
                                    echo '<option '.$a.' value = "'.$id.'">'.$type_name.'</option>';
                                }
                            ?>
                        </select>
                    </div>        
                ...
                <div class = label><label for = "corresp_weight">Вес</label>
                    <div>
                        <input id = "corresp_weight" type = "number" step = 'any' name = "corresp_weight" value = "<?php echo $corresp['correspWeight']; ?>" required/>
                    </div>        
                </div>
